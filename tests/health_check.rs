...
async fn spawn_app() -> TestApp{
    ...
    let configuration = get_configuration().expect("Failed to read configuration."); let connection_pool = PgPool::connect(
...
//     let server = run(listener).expect("Faild to spawn server.");
//
//     let _ = tokio::spawn(server);
